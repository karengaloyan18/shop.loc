...
{{--        {{$frs}}--}}
{{--    {{$frst}}--}}
{{--            @foreach($frs as $fr => $fs)--}}

<div class="h3"> {{$frst . ' =>' . $frs}} </div>
@csrf
                            <input type="hidden" name="_token" value="{{ csrf_token() }}"/>
                            <button style="color: #c82333" type="button"   class="btn btn-group-vertical del h3"
                                    value="{{$frst}}" name="{{$frs}}">Удалить</button>
{{--            @endforeach--}}
@endforeach
